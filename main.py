...
# INPUT (1)
method = 3 # or 3 dots to calculate the velocity (does not influence anything here)

# INPUT (2, 3 and 4)
mode = 1 # related to how the depth of the crater is calculated (1 is a good value)
...
ej.main(path,folders,pathdata,method,thresholdf,g) # Ve, de, De and shape of cavities 
...
for idx, modelname in enumerate(folders):
    ...
    readd.param(pathi, poros, paths + modelname + '/data/',nlayers) # generate python_param
    #export.scalinglawsDRIM(paths + modelname + '/data/', modelname, nlayers) # generate scalinglaws DRIM
    export.scalinglaws(paths + modelname + '/transient/', modelname, nlayers) # generate scalinglaws


name = 'HOM_AUG'
search = 'C*'
export.tablepi(paths,search,name, nlayers) # generate a table with L, pi2, pi3, piD ....
...
export.savefactors(paths + 'piDpiV/', lst, name, Dsc) # take previously generated file and calculate factors
