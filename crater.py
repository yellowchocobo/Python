...
def appvolume(model,step,time,id_mat,oridy,diam,ix):
    ...
    if time == 0:
        ...
        R = model.x[i,j] + ((1 - step.cmc[id_mat][i,j]) * (model.x[ii,j] - model.x[i,j]))
...
def maxappdepth(model,step,time,id_mat,oridy,diam,ix):
    ...
    if (time == 0) or (ix == 0):
        ...
        empty_cells = np.where((conc >0) & (conc < 1)) # here I have the index
        ...
        if empty_cells[0].size == 0:
            ...
            depth = np.abs((model.y[iif,ijf+1]) - ((1 - step.cmc[0][iif,ijf]) * model.dy))
...
def midappdepth(model,step,time,id_mat, diam):
    ...
    if time == 0:
        ...
        depth = np.abs((model.y[0,ijf]) - ((1 - step.cmc[0][0,ijf-1]) * model.dy))
...
def emptycells(model,step,time,oridy,diam,ix):
    ...
    if time == 0:
        ...
        ec = np.sum((sl== 0))
        pf= np.sum(sl[np.where((sl > 0) & (sl < 1))]) #sum of concentration of matter
        ls = np.sum(sl<1)
...
def rimrim(model,step,time,id_mat, oridy, diam,ix):
    ...
    if time == 0:
        ...
        # otherwise, we take the maximum y-index (with the biggest height)
            ...
            alt = np.abs((model.y[iif,ijf+1]) - ((1 - step.cmc[0][iif,ijf]) * model.dy))
    ...
    for u in range(model.nsteps):  
        ...
        if time[u] == 0:
            ...
            #otherwise we go through all the calculations
                ...
                vol[u] = appvolume(model,step,time[u],id_mat,oridy,diam[u],ix)
                
                # if mode = 0, we calculate the apparent depth in two different ways
                if mode == 0:                
                    depth[u] = midappdepth(model,step,time[u],id_mat,diam[u])
                    mdepth[u] = maxappdepth(model,step,time[u],id_mat,oridy,diam[u],ix)
                # if mode = 1, we calculate only the maximum apparent depth
                elif mode == 1:
                    mdepth[u] = maxappdepth(model,step,time[u],id_mat,oridy,diam[u],ix)
                
                # last functions
                alt[u], drim[u], ixxx, iyyy = rimrim(model,step,time[u],id_mat, oridy,diam[u],ix)
                Vrim[u] = rimvolume(model,step,time[u],id_mat,diam[u],ixxx,iyyy)
                ec[u], ls[u], pf[u] = emptycells(model,step,time[u],oridy,diam[u],ix)
...
def main(path,folders,paths,mode,id_mat, transient):
    ...
    for modelname in folders:
        ...
        time, depth, mdepth, diam, vol, ec, ls, pf, alt, drim, Vrim = craterdimensions(model,mode,id_mat)
        ...
        # output file
        output = np.column_stack((time, depth, mdepth, diam, vol, ls, ec, pf, alt, drim, Vrim))                                                 
        ...
        if mode == 0:       
            output2 = np.column_stack((time[idx_tr], depth[idx_tr], diam[idx_tr], vol[idx_tr], alt[idx_tr], drim[idx_tr], Vrim[idx_tr],idx_tr))                                                 
            np.savetxt(fname, output2, header = header_txt ,
                       delimiter='\t', fmt=['%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e'])
        else:
            output2 = np.column_stack((time[idx_tr], mdepth[idx_tr], diam[idx_tr], vol[idx_tr], alt[idx_tr], drim[idx_tr], Vrim[idx_tr],idx_tr))                                                 
            np.savetxt(fname, output2, header = header_txt ,
                       delimiter='\t', fmt=['%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e'])
        ...
        if tnorm[np.where(~np.isnan(diam))[0][-1]] >= 9: 
            ...
            if mode == 0:       
                output2 = np.column_stack((np.nanmean(time[stp1]), np.nanmean(depth[stp1]), np.nanmean(diam[stp1]), np.nanmean(vol[stp1]), np.nanmean(alt[stp1]), np.nanmean(drim[stp1]), np.nanmean(Vrim[stp1])))                                                 
                np.savetxt(fname, output2, header = header_txt ,
                           delimiter='\t', fmt=['%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e'])
            else:
                output2 = np.column_stack((np.nanmean(time[stp1]), np.nanmean(mdepth[stp1]), np.nanmean(diam[stp1]), np.nanmean(vol[stp1]), np.nanmean(alt[stp1]), np.nanmean(drim[stp1]), np.nanmean(Vrim[stp1])))                                                 
                np.savetxt(fname, output2, header = header_txt ,
                           delimiter='\t', fmt=['%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e'])
                
        else:
            if mode == 0:       
                output2 = np.column_stack((np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan))                                                 
                np.savetxt(fname, output2, header = header_txt ,
                           delimiter='\t', fmt=['%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e','%1.6e'])
            else:
                output2 = np.column_stack((np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan))                                               
...
def rimrimProfile(model, step, drim, id_mat):
    ...
    for values in np.arange(len(jj)):
        ...
        a1 = (model.y[ii[values],jj[values]+1]) - ((1 - step.cmc[0][ii[values],jj[values]]) * model.dy)
...
def craterProfiles(folders, path, paths, idx, mode):
    ...
    for ix, modelname in enumerate(folders):
        ...
        if mode == 1:
            __, __, __, __, __, __, __,idx_tr = np.loadtxt(paths + modelname + '/transient/' + modelname + '_tr.txt')
...
def craterProfileXY(model, stp, id_mat, layerdepth):
    ...
    for values in np.arange(len(jj)):
        ...
        a1 = (model.y[ii[values],jj[values]+1]) - ((1 - step.cmc[id_mat][ii[values],jj[values]]) * model.dy)
        a2 = (np.abs(model.xc[ii[values],jj[values]]))
        altr = np.append(altr,a1)
        rrim = np.append(rrim,a2)
...
def morph2layers(model,t):
    ...
    # load last step   
    den = model.readStep('Den',model.nsteps-1) # density seems to be better : ) e.g with C20P00F08_L100
    step = copy.deepcopy(den)
    ...
    MD_UL = maxappdepth(model,step,time,0,oridy,ix) # dapp to all material
    ...
    MD_LL = maxappdepth(model,step,time,2,oridy,ix3) # dapp to second material
    HRIM_LL, DRIM_LL = rimrim(model,step,time,2,oridy2,ix3) # it gives a positive depth (but it should be negative) DRIM mat 2 (it does not manage to get the rim)
