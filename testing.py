...
plt.hlines(model.yhires[0]/a,model.xhires[0]/a,model.xhires[1]/a,'k',linewidth=2)
plt.hlines(model.yhires[1]/a,model.xhires[0]/a,model.xhires[1]/a,'k',linewidth=2)
plt.vlines(0,model.yhires[0]/a,model.yhires[1]/a,'k',linewidth=2)
plt.vlines(model.xhires[1]/a,model.yhires[0]/a,model.yhires[1]/a,'k',linewidth=2)

plt.hlines(1000./1000.,model.xhires[0]/a,model.xhires[1]/a,'r',linewidth=4) # Z = 0.1 R
...
xtime = np.linspace(0,dt*199.,199.) #because of the tracers script the last step is not saved

xtime = np.linspace(0,dt*99.,99.) #because of the tracers script the last step is not saved
...
plt.scatter(xpos/2000.,v/12700.,c=timesteps,cmap='jet',vmin=0,vmax=100,s=4,linewidths=0)
plt.colorbar()
#plt.xscale('log')
plt.yscale('log')
...
plt.scatter(xpos/10000.,angle,c=timesteps,cmap='jet',vmin=0,vmax=200,s=4,linewidths=0)
plt.colorbar()
plt.xscale('log')
#plt.yscale('log')
plt.xlabel('xpos/R',fontsize = 16)
...
# xpos vs angle (with timesteps in angles) ## FIGURE 3 ##
plt.scatter(v/12700.,angle,c=timesteps,cmap='jet',vmin=0,vmax=200,s=4,linewidths=0)
...
# FIG 2
import matplotlib
plt.scatter(xpos/2000.,angle,c=v/12700.,cmap='jet',vmin=1e-3,vmax=1,s=4,linewidths=0,norm=matplotlib.colors.LogNorm())
clb = plt.colorbar()
...
# FIG 4
import matplotlib
plt.scatter(v/12700.,angle,c=dland/2000.,cmap='jet',vmin=1,vmax=1000,s=4,linewidths=0,norm=matplotlib.colors.LogNorm())
clb = plt.colorbar()
clb.set_label(r'$land_{pos}/a$', fontsize = 14, labelpad=-35, y=1.075, rotation=0)
...
# FIG 5
import matplotlib
plt.scatter(v/12700.,angle,c=tair,cmap='jet',vmin=1.0,vmax=2000,s=4,linewidths=0,norm=matplotlib.colors.LogNorm())
clb = plt.colorbar()
...
# last
plt.pcolormesh(model.xc/(10000.),model.yc/(10000.),stepend.mat,cmap='Pastel1')
...
'''
# all three of them
plt.scatter(xpos/10000.,angle,c=v/12700.,cmap='jet',vmin=0.1,vmax=0.5,s=4,linewidths=0)
plt.colorbar()
#plt.xscale('log')
#plt.yscale('log')
plt.xlabel('xpos/R',fontsize = 16)
...
plt.plot(step1.xmark[model.tru[0].start:model.tru[0].end], step1.ymark[model.tru[0].start:model.tru[0].end],'bo')
plt.plot(step1.xmark[model.tru[1].start:model.tru[1].end], step1.ymark[model.tru[1].start:model.tru[1].end],'ro')
...
for xo in range(np.shape(ikok)[1]):
    ...
    locTracers.append(step1.findTracer(a,b)) # findTracer take x and y coordinates
...
ixox = np.where((step1.data[0] < 2680.) & (stepSie.data[0] < 3.5e6) & (model.yc >= 1000.) & (step1.cmc[0] > 0))
ixox2 = np.where((step1.data[0] < 2680.) & (stepSie.data[0] < 3.5e6) & (model.yc <= 1000.) & (step1.cmc[0] > 0))
